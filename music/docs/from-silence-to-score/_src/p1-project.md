...
## P1.2 Four decisions before the first note
...
1. **Clef** (Chapter 5). For a melody in the range of a voice or the right hand, the **treble clef** is almost always right. "Ode to Joy" lives comfortably around and just below the middle of the treble staff.
2. **Key** (Chapter 8). Find the home note — the pitch the tune keeps returning to and ends on — and the scale it draws from. "Ode to Joy" is in **D major**, two sharps. Setting that key signature now means every F and C you type will be sharp automatically (Chapter 8's whole point), so the notation stays clean.
3. **Meter** (Chapter 6). Tap along and feel the grouping. "Ode to Joy" is a steady **4/4** — four beats to the bar, no lilt.
4. **Tempo** (Chapter 10). Judge the speed from the recording. A moderate walking pace, about **♩ = 112**, suits it.
...
## P1.3 Entering the notes

Now the core loop from Chapter 2: **choose a duration, then type the letters.** Work one phrase at a time, checking each against the recording as you go. For the first phrase of "Ode to Joy," the rhythm is steady quarter notes until the very end of the fourth bar, so press <kbd>5</kbd> for a quarter and type the pitches — and because the key signature is doing the accidental work, you type plain letter names even for the sharps:

{{figure ode-plain | The first phrase of "Ode to Joy," pitches and rhythm only. Four bars in D major, 4/4 — F♯ F♯ G A, A G F♯ E, D D E F♯, then a dotted-quarter, eighth, and half to close. Note that not one accidental clutters the notes: the two-sharp key signature makes every F an F♯ on its own, exactly as Chapter 8 promised. The dotted rhythm in the last bar (Chapter 6) is the only place the steady quarters break.}}

The one rhythmic subtlety is that closing bar in {{fig ode-plain}}: a dotted quarter, an eighth, and a half note. Set it with the duration keys — <kbd>5</kbd> then <kbd>.</kbd> for the dotted quarter, <kbd>4</kbd> for the eighth, <kbd>6</kbd> for the half — and type the pitches. Then enter the second phrase the same way; it repeats the first almost exactly, ending on D instead of E. When the pitches or rhythm fight you, slow the recording down (Chapter 3's Speed slider) and check note by note.

## P1.4 From correct to musical
...
That is the whole arc of the project in one comparison: {{fig ode-plain}} is *correct*, {{fig ode-to-joy}} is *finished*. The difference is entirely the expression layer, and it is the difference between a data file and a piece of music someone would want to play.

## P1.5 Proofread, save, export

Before you call it done, **check it against the recording one more time** — play your score back (<kbd>Space</kbd>) and, better still, play the recording and your MuseScore version back to back. Errors that are invisible on the page are obvious to the ear: a wrong pitch, a rhythm that does not line up, a missed sharp. Fix what you hear.
...
!!! note "Done when…"
    ...
    - The file is **saved as `.mscz`** and a **PDF is exported**.
    - A stranger could read your score and play the tune back — recognizably.
