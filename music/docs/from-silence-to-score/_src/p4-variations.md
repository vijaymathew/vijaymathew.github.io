...
{{figure variation-theme | A simple theme in C major. Plain quarter notes, an obvious harmony (C, then G to C), a clear little shape rising and falling. Its very plainness is the point: it is a blank canvas, memorable enough to track through the variations to come, simple enough that there is everywhere to take it.}}

The theme in {{fig variation-theme}} is nobody's masterpiece, and that is exactly right. Everything interesting will happen *to* it.

## 24.2 What stays, and what changes
...
{{figure variation-figured | **Variation by melodic figuration.** The theme's notes still fall on the beats — C, E, G, E in the first bar, exactly as before — but now passing tones fill the gaps between them, and the melody flows in running eighths where it once stepped in quarters. The harmony beneath is unchanged; only the surface is busier. The theme is still there, just decorated.}}

{{figure variation-minor | **Variation by mode change.** The same rising-and-falling contour as the theme, but its third and sixth are lowered to turn C major into C minor. The notes trace the identical shape; the colour is transformed from bright to shadowed. This one gesture — major to minor — is among the most affecting a variation can make.}}

Compare {{fig variation-figured}} and {{fig variation-minor}} with the plain theme and the method is clear: in the first, the theme's notes are preserved and decorated; in the second, its shape is preserved and recoloured. Each holds onto the theme by a different thread, and each sounds new while remaining, audibly, the same tune.

## 24.4 Building the set
...
!!! muse "In MuseScore"
    ...
    1. **Write the theme** once, with its harmony (chord symbols or a simple accompaniment).
    2. **Copy the whole theme** (<kbd>Ctrl</kbd>+<kbd>C</kbd>) and paste it below or after, as many times as you want variations. Each copy starts identical to the theme, with the harmony already in place.
    3. **Transform each copy** with the tools you know: rewrite the melody in faster notes for a figuration variation; use **Tools ▸ Transpose** or the arrows to lower the thirds and sixths for a minor variation; rewrite just the accompaniment staff for a textural variation. Because the copy already carries the theme's chords, you always have the harmonic frame to build on.
    4. Play each variation against your memory of the theme, and against the others, checking that it is recognizable yet distinct.

    **Try it:** enter the plain theme of {{fig variation-theme}} with its chords, then copy it twice. Turn the first copy into the running-eighths figuration of {{fig variation-figured}}, and the second into the minor version of {{fig variation-minor}} by flattening its thirds. Play all three in a row. You have written your first variation set — theme, a figured variation, and a minor one — and felt how one plain idea can be remade without limit.
