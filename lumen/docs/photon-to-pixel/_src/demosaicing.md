...
Green plus color differences is the `gradient` method, and it buys nearly five decibels over bilinear on the torture target — the single largest step anywhere on this chapter's ladder. Almost every serious demosaic since the mid-1990s, however elaborate, has this exact structure at its core: dense channel first, differences after.
