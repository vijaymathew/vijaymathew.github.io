...
{{figure first-light | First light: a linear grayscale ramp over the eight corners of the RGB cube — drawn pixel by pixel with the `Image` class and written by our own `write_png`. This unglamorous test card, grown into full spectral color charts and detail targets, is the seed of the simulated camera in Part 1.}}

If the ramp looks like it brightens unevenly — too dark for too long, then rushing to white — you have just observed, with your own eyes, the difference between linear light and your display's expectations. That observation is Part 7's opening argument, and we made it with sixty lines of standard-library Python.
...
{{figure exposure-stops | One stop down, as shot, one stop up: every channel value halved or doubled, nothing more. At +1, the upper ramp's brighter half has sailed past 1.0 and been clipped to white by `write_png` — our first clipped highlight, and an honest preview of why Part 7 spends a whole section on recovering them.}}
