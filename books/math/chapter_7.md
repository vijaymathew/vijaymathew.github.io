...
![A geometric explanation of sine for astronomy, showing a circle, a central angle, the right triangle used to define sine, and a comparison between simple angles and arbitrary angles that astronomers actually need.](images/kerala_sine_astronomy.svg){fig-alt="A two-panel diagram explaining why accurate sine values mattered in Kerala astronomy. The left panel shows a circle of radius R, a central angle theta, and a right triangle with the opposite side highlighted, together with the formula sine of theta equals opposite over radius. The right panel contrasts easy angles like 30, 45, and 60 degrees with an arbitrary angle measured in degrees, minutes, and seconds, and shows how a small sine error becomes a larger angular error in a predicted planetary position." width="94%"}
