...
The right approach is to understand what each tool is optimised for, what it costs to use it, and what it produces — then match the tool to the document.
...
**LaTeX** is the gold standard for print-quality typesetting of complex documents, especially those with heavy mathematical content. It is a typesetting system first and a document format second — the author writes markup that instructs the typesetting engine, and the engine makes sophisticated decisions about line breaking, hyphenation, spacing, and page layout. LaTeX's paragraph-level optimisation produces text that looks measurably better than the output of simpler systems. Its ecosystem of packages is vast: virtually any typographic requirement has been addressed by some package somewhere on CTAN.
...
LaTeX is the right choice when: typographic quality is paramount; the document contains significant mathematical content; you are submitting to a publisher or conference that requires LaTeX input; you need a stable, reproducible toolchain over years or decades; or you are building on a substantial existing LaTeX infrastructure.
...
**Typst** is a modern document composition system designed to do what LaTeX does while being substantially more approachable. Its syntax is cleaner and more consistent than LaTeX's, its error messages are informative, its compilation is fast (it compiles incrementally, so large documents update in milliseconds rather than seconds), and its scripting language — a proper functional language built into the tool — makes programmatic document generation natural.

Typst is the right choice when: you want LaTeX-quality output without LaTeX's complexity; you are starting a new project without legacy LaTeX infrastructure to integrate; you need programmatic document generation (loop over data, generate tables, compute values); or you are writing in a domain where Typst's ecosystem is already strong.
...
LaTeX's typographic quality score reflects the quality ceiling, not the floor. Default LaTeX with Computer Modern at 10pt and stock settings is already good; LaTeX with careful font selection, `microtype` for protrusion and expansion, and attention to spacing is the best freely available typesetting system in existence. Typst is close and closing the gap; it produces noticeably better output than systems without paragraph-level optimisation.
...
*Is the document primarily mathematical — theorems, proofs, substantial notation?* If yes, and if you are starting fresh with no legacy constraints: Typst is increasingly viable and worth evaluating. If you need maximum ecosystem compatibility or are joining an existing mathematical project: LaTeX. If no: continue.

*Do you need more than one output format from the same source — for instance, both a PDF and a website?* If yes: Pandoc or Quarto (Quarto if the document has any computational content; Pandoc for pure prose). If no: continue.

*Is the document long, complex, and primarily a print artifact — a book, a formal report, a thesis?* If yes: LaTeX for maximum typographic control, or Typst for a more modern authoring experience with equivalent output quality. If no: continue.

*Is it a short-to-medium prose document — an article, a report, documentation?* Pandoc Markdown with appropriate defaults is the simplest path. Typst is an excellent alternative for documents that will be PDF-only.
...
**Quarto for computation, LaTeX for typesetting.** Quarto can output to LaTeX, which can then be compiled with a full LaTeX engine. This combines Quarto's code execution with LaTeX's typographic quality — the approach used by several academic publishers and many researchers producing high-quality papers.
...
**Make to orchestrate multiple tools.** A `Makefile` can invoke LaTeX for the print PDF, Pandoc for HTML and EPUB, and Quarto for a companion website, all from overlapping source files. The build system is the integration layer that makes the combination tractable.
...
The framework in this chapter — asking about output format requirements, document complexity, and workflow constraints — will remain useful even as the specific tools change. What changes is which tool best satisfies each combination of requirements. As of this writing, the assessments above reflect the current state of the tools; in two or three years, Typst's ecosystem may have grown to the point where it displaces LaTeX for more use cases, or a new tool may have appeared that changes the landscape again.
